...
DELIMITER $$
create procedure filterActive(
    in p_nombreVendor varchar(50),
    in p_claseActivo varchar(50),
...
begin
    select 
        ...
        v.nombreVendor
    from activo a
    join detalleRecepcion dr on a.idActivo = dr.idActivoFk
    join vendor v on dr.idVendorFK = v.idVendor
    where
        (p_nombreVendor is null or v.nombreVendor = p_nombreVendor)
        and (p_claseActivo is null or a.claseActivo = p_claseActivo)
        and (p_fechaInicio is null or a.timestampRecepcion >= p_fechaInicio)
        and (p_fechaFin is null or a.timestampRecepcion <= p_fechaFin);
end$$
DELIMITER ;
describe detalleRecepcion;
describe activo;

call filterActive('Factset','SHARE', NULL, NULL);
call filterActive("ICE","share",'2025-06-01','2025-06-30 23:59:59');
call filterActive("EDI","share",'2025-06-01','2025-06-30 23:59:59');

select*from vendor;

/*requisito 4 funcion

-no se puede, porque las funciones nos dan un dato que podria ser (varchar,int,timestap,determisitc) pero no 
una tabla filtrada sin columna inecesarias. En el proyecto se depurara la base de datos en phyton antes de 
subirlo en SQL
*/
...
DELIMITER $$
create function depuracion(fIdActivo varchar(50),
							fPrecio int,
                            ticker varchar(50)
                            )
returns varchar(255)
